...
- **Private Ad Detection via search-page JSON (no detail pages)**: The private-vs-agency signal is read from the search page's embedded Vue state (`browseListingsStore`) via the `isOwnerResidentialSeller` boolean (`true` = Privatni, `false` = Agencija) — parsed by `parseListingsFromSearchJSON` in `njuskalo.ts`. This flag maps exactly to the detail-page phrase "Korisnik nije trgovac" and is present even on promoted/SuperVau cards.
- **Land Detail Page Fetching**: For land categories (`zemljista` and `najam_zemljista`), `monitor.ts` explicitly fetches the detail page via Playwright (`fetchDetailPagePlaywright`) to extract `land_type` and `size_m2` directly from the detail page structured data ("Tip zemljišta") to ensure 100% classification accuracy.
